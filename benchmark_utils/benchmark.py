...
def benchmark(func: Callable, num_repeats: int, progress_bar: Progress, name: str = '') -> List[float]:
...
class Benchmark:
    ...
    def _run(self, bench_func_dict: dict, num_repeats: Union[int, None] = None) -> None:
        ...
        with Progress(transient=True) as progress_bar:
            main_task = progress_bar.add_task("starting...", total=num_funcs)
            for num, func_name in enumerate(func_names):
                # progress_bar.set_description(f"running {func_name}")
                progress_bar.tasks[main_task].description = f"{text_color}running {func_name} {num + 1}/{num_funcs}"
                self._results[func_name] = self._run_benchmark(
                    func_name, num_repeats=num_repeats, progress_bar=progress_bar)
                # progress_bar.update(1)
                progress_bar.update(main_task, advance=1)

        self.print_results()

    def _run_benchmark(self, func_name: str, num_repeats: int, progress_bar: Progress):
        return self._benchmark(self.bench_func_dict[func_name], num_repeats, progress_bar, func_name)
...
class BenchmarkIter(Benchmark):
    ...
    def _run_benchmark(self, func_name: str, num_repeats: int, progress_bar: Progress):
        return self._benchmark(self.run_func_iter(func_name), num_repeats, progress_bar, func_name)
